...
const SpindleControlModal: React.FC = () => {
  // ✅ 컴포넌트 이름 수정
  const { rotationSpeed, setRotationSpeed, resetRotationSpeed } =
    useRotationSpeedStore();
  ...
  return (
    <Dialog.Content className="text-[100%] fixed top-1/2 left-1/2 bg-backgroundPrimary p-6 rounded-lg border border-white h-[36vh] w-[20vw] shadow-xl transform -translate-x-1/2 -translate-y-1/2 animate-fade-inv">
      ...
      <Dialog.Title className="text-[200%] text-primary font-bold mb-titleBottomMargin">
        Spindle Control
      </Dialog.Title>
        ...
        <div className="flex flex-col gap-gridGap h-[20%]">
          ...
          <div className="flex items-center">
            <input
              ...
              className="flex-1 h-[3.5vh] p-2 border border-gray-500 rounded bg-gray-800 text-white text-right focus:outline-none focus:border-white focus:border-1 focus:shadow-md [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
            />
      ...
      <div className="w-[100%] h-full mx-auto flex flex-col justify-between">
        <button className="h-[12%] w-full bg-primary text-white px-4 py-2 rounded-md cursor-pointer text-lg font-bold transition-colors duration-200 mt-applyButtonTopMargin hover:bg-buttonHover flex items-center justify-center text-center">
          Apply
        </button>
      </div>
    </Dialog.Content>
